...
<div className="grid grid-cols-12 lg:gap-8 gap-3 gap-y-4 lg:mx-20 mx-4 pb-10">
  ...
  <div className="pt-5 w-full text-center">
    <h2 className="lg:text-xl text-base font-bold ">{item.strCategory}</h2>
    <p className=' text-sm rounded-md py-1 px-2 hidden lg:inline-block  text-orange-400 '>{item.strCategoryDescription.split(" ").slice(0,18).join(" ")}</p>
    <p className=' text-sm rounded-md py-1 px-2 lg:hidden  text-orange-400 inline-block'>{item.strCategoryDescription.split(" ").slice(0,10).join(" ")}</p>
  </div>
