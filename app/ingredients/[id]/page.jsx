import { IngredientFilter } from '@/app/_util/fetchdata'
import Image from 'next/image'
import Link from 'next/link'
import React from 'react'

export default async function page({params}) {
   const data = await IngredientFilter(params.id)
...
<figure><Image src={item.strMealThumb} alt="food-image" width={150} height={150} className='rounded-full h-32 w-32 shadow-lg'/></figure>
<div className="pt-5 w-full text-center">
<h2 className="lg:text-xl md:text-sm text-[12px] font-bold ">{item.strMeal}</h2>
</div>
<div className=' my-2'>
